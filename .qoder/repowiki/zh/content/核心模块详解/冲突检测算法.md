...
- **新增**：双重防御机制详解——反瞬移过滤（tau_teleport）和事件确认时间窗（confirm_window/min_confirm_frames）
- 完整流程说明：输入数据准备、特征计算、状态转换、结果输出
...
RULES["interaction_rules.py<br/>规则与状态机<br/>含双重防御机制"]
end
CFG --> RULES
YML --> RULES
CT --> PAIR
...
- **增强**：双重防御机制——反瞬移过滤（tau_teleport）和事件确认时间窗（confirm_window/min_confirm_frames）
...
### 反瞬移过滤（Tau Teleport Protection）

**新增功能**：反瞬移过滤是系统的第一道防线，专门检测和过滤由YOLO关键点闪烁导致的非人类物理现象。

#### 核心原理
- **物理限制**：人类不可能在一帧内移动超过3倍肩宽的距离
- **阈值设定**：使用归一化特征值（r_a 和 r_v）作为判断标准
- **实时检测**：在每帧状态机更新前进行瞬移检测

#### 实现细节
```python
# 检测瞬移现象
is_teleport_ab = details_ab.get("r_a", 0) > self.tau_teleport or details_ab.get("r_v", 0) > self.tau_teleport
is_teleport_ba = details_ba.get("r_a", 0) > self.tau_teleport or details_ba.get("r_v", 0) > self.tau_teleport
...
#### 配置参数
- `tau_teleport`：瞬移阈值，默认值为0.95
- **注意**：配置文件中的默认值为15.0，这可能是配置文件中的错误标注

### 事件确认时间窗（Confirm Window）

**新增功能**：事件确认时间窗是系统的第二道防线，确保冲突检测的时序一致性。

#### 核心原理
- **时间连续性**：冲突必须在连续的时间窗口内持续满足作用-响应条件
- **统计验证**：通过历史记录验证冲突的真实性
- **防误报**：防止瞬时异常导致的误判
...
# 只有当满足条件的帧数达到阈值时才确认冲突
if current_response and response_count >= self.min_confirm_frames:
    self.state = 3  # 进入事件确认阶段
```

#### 配置参数
- `confirm_window`：确认时间窗大小，默认值为4帧
- `min_confirm_frames`：最小确认帧数，默认值为3帧

```mermaid
flowchart TD
Start(["开始检测"]) --> Teleport["反瞬移过滤"]
Teleport --> Valid{"瞬移检测通过？"}
Valid --> |否| Block["阻断状态升级"]
Valid --> |是| Response["检查作用-响应条件"]
Response --> History["更新历史记录"]
History --> Count["统计满足条件的帧数"]
Count --> Threshold{">= min_confirm_frames？"}
Threshold --> |否| Hold["保持当前状态"]
Threshold --> |是| Confirm["确认事件"]
Block --> End(["结束"])
Hold --> End
Confirm --> End
```
...
**图表来源**
- [src/fightguard/detection/interaction_rules.py:294-410](file://src/fightguard/detection/interaction_rules.py#L294-L410)

**章节来源**
- [src/fightguard/detection/interaction_rules.py:275-282](file://src/fightguard/detection/interaction_rules.py#L275-L282)
- [src/fightguard/detection/interaction_rules.py:294-410](file://src/fightguard/detection/interaction_rules.py#L294-L410)
...
- **新增**：反瞬移过滤误伤
  - 现象：正常快速动作被误判为瞬移
  - 处理：适当提高 tau_teleport 阈值，或检查关键点质量
- **新增**：事件确认时间窗过于严格
  - 现象：真实冲突被忽略
  - 处理：降低 min_confirm_frames 或增大 confirm_window
...
- **新增**：tau_teleport（反瞬移阈值）：检测瞬移现象的阈值
- **新增**：confirm_window（确认时间窗）：事件确认的时间窗口大小
- **新增**：min_confirm_frames（最小确认帧数）：确认事件所需的最少帧数

**章节来源**
- [configs/default.yaml:16-34](file://configs/default.yaml#L16-L34)
- [src/fightguard/detection/interaction_rules.py:363-408](file://src/fightguard/detection/interaction_rules.py#L363-L408)
- [src/fightguard/detection/interaction_rules.py:410-503](file://src/fightguard/detection/interaction_rules.py#L410-L503)
...
- rules
  ...
  - 关键项：proximity_threshold、wrist_intrusion_threshold、velocity_threshold、conflict_duration_frames、alert_threshold、proximity_window_frames、smoothing_window_frames、teacher_presence_threshold、tau_c、tracker、tracker_conf 等。
...
## 结论
KidGuard 的配置管理系统通过单一可信源与模块级缓存，实现了对路径、规则、数据集与输出行为的集中管理。配合严格的校验与清晰的错误提示，开发者可在不重启进程的前提下进行热更新与快速调参，显著提升开发与评测效率。建议在团队内统一参数命名规范与阈值设置流程，确保配置的一致性与可维护性。

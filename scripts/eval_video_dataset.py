"""
eval_video_dataset.py — 真实视频数据集批量评测脚本
====================================================
用于在真实的 2D 监控视频数据集（如 Five-Fight）上评估规则引擎的泛化能力。
揭露系统从 3D 迁移到 2D 后的真实漏报与误报情况，为下一步状态机重构提供基线数据。
"""
...
def evaluate_on_videos(num_samples_per_class: int = 5):
    print("="*60)
    print(f" 真实视频数据集 Baseline 评测 (每类抽取 {num_samples_per_class} 个)")
    print("="*60)
    ...
    # 随机抽样
    random.seed(42) # 固定随机种子，保证每次抽样一致，便于对比
    ...
    for video_path, actual_label in tqdm(test_cases, desc="视频处理进度"):
        # 提取骨骼 (开启追踪)
...
if __name__ == "__main__":
    evaluate_on_videos(num_samples_per_class=5)
